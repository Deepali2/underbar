(function () {
  ...
  _.uniq = function (array, isSorted, iterator) {
    ...
    if (!isSorted) {
      var sortedArr = arrayCopy.sort();
    } else {
      var sortedArr = arrayCopy;
    }
    var convertor = iterator || function (element) { return element };
    var iteratedArr = sortedArr.filter(element => convertor(element));
    var lastElement;
    iteratedArr.forEach((currentElement) => {
  ...
  _.zip = function () {
    var zipped = [];
    var longestArr = [].sort.call(arguments, (x, y) => {
    ...
    for (var i = 0; i < arguments[0].length; i++) {
      for (var j = 1; j < arguments.length; j++) {
        if (!(_.contains(arguments[j], arguments[0][i]))) {
          notIntersected.push(arguments[0][i]);
        }
        if (!(_.contains(notIntersected, arguments[0][i]))) {
          intersected.push(arguments[0][i]);
        }
      }
    }
    return intersected;
  ...
  _.difference = function (array) {
    ...
    for (var i = 0; i < arguments[0].length; i++) {
      for (var j = 1; j < arguments.length; j++) {
        if ((_.contains(arguments[j], arguments[0][i]))) {
          notDiff.push(arguments[0][i]);
        }
      }
      if (!(_.contains(notDiff, arguments[0][i]))) {
  ...
  _.throttle = function (func, wait) {
    ...
    return function () {
      var now = Date.now();
      counter = now - prev;
      if (counter > wait || calledOnce === true) {
        calledOnce = true;
        prev = null;
